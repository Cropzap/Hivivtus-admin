...
const TicketDetailView = ({ ticket, onClose, onReply, onStatusChange, isProcessing, adminUserId }) => {
  ...
  const handleReplySubmit = async (e) => {
    ...
    if (replyMessage.trim()) {
      // Pass the MongoDB _id, not the ticketId
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 bg-white shadow-2xl z-20 flex flex-col rounded-3xl overflow-hidden"
    >
            ...
            {ticket.replies?.length > 0 ? (
              ticket.replies.map((reply, index) => (
                <div key={index} className={`flex items-start gap-3 ${reply.userId === adminUserId ? 'justify-end' : ''}`}>
                  {reply.userId !== adminUserId && <div className="p-2 bg-blue-500 text-white rounded-full"><FaUser className="h-4 w-4" /></div>}
                  <div className={`p-3 rounded-xl max-w-md shadow-sm ${reply.userId === adminUserId ? 'bg-emerald-100' : 'bg-gray-200'}`}>
                  ...
                  {reply.userId === adminUserId && <div className="p-2 bg-emerald-600 text-white rounded-full"><FaUserTie className="h-4 w-4" /></div>}
                </div>
              ))
            ) : (
      ...
      <div className="p-4 border-t bg-gray-50 rounded-b-3xl">
        <form onSubmit={handleReplySubmit}>
          <textarea
            ...
            className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all resize-none"
            disabled={isProcessing}
          />
          <motion.button
            type="submit"
...
export default function AdminSupportDashboard() {
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(ticket => {
      ...
      const matchesSearch = filters.search === '' ||
        ...
        (ticket.buyerName && ticket.buyerName.toLowerCase().includes(searchLower));
  ...
  const handleReply = useCallback(async (_id, replyMessage) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}support-tickets/${_id}/reply`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
  ...
  const handleStatusChange = useCallback(async (_id, newStatus) => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      // Use the MongoDB _id in the API call
      const response = await fetch(`${API_URL}support-tickets/${_id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${token}`,
                  ...
                  {paginatedTickets.length > 0 ? (
                    paginatedTickets.map(ticket => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{ticket.buyerName || 'Unknown'}</td>
